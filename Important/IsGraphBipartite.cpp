...
public:
    bool isBipartite(vector<vector<int>>& graph) {
       vt<int> color(graph.size(), 0);  // Initialize color array with zeros
       F_OR1(graph.size()) {
           if (color[i] == 0 && !validcolor(graph, color, 1, i))  // Pass graph, color array, and initial color (1)
               return false;
       }
       return true;
    }
    
    bool validcolor(vt<vt<int>>& graph, vt<int>& color, int c, int node) {
        if (color[node] != 0)
            return color[node] == c;
        color[node] = c;
        for (int n : graph[node]) {
            if (!validcolor(graph, color, -c, n))  // Invert the color when recursively calling validcolor
                return false;
        }
        return true;
    }
