...
private:
	void solveNQueensUtil(vt<vt<string>>&result,vt<string>&board,int col,int n){
		if(col==n){
		...
		F_OR2(row,n){
			if (isSafe(board,row,col,n)){
				//place the queen in (row,col) position
				board[row][col]='Q';
				//recur to place the rest of the queens
				solveNQueensUtil(result,board,col+1,n);
				//backtrack and remove the queen from (row,col) position
				board[row][col]='.';

			}
		}
	}
	bool isSafe(const vt<string>&board,int row,int col,int n){
		//checking for queens in same column
		F_OR1(col){
			if(board[row][i]=='Q'){
				return false;
			}
...
Solution solution;
int n=4;
vt<vt<string>>result=solution.solveNQueens(n);
