...
public:
        int maxTime=INT_MIN;
        void dfs(vt<int>adjList[],vt<int>&informTime,int current,int time){
            //max time for an employee to get the news
            maxTime=max(maxTime,time);

            for(int adjacent:adjList[current]){
                //visit the subordinate employee who gets the news after informTime[curr] unit time
                dfs(adjList,informTime,adjacent,time+informTime[current]);
         ...
         int numOfMinutes(int n, int headID, vector<int>& manager, vector<int>& informTime) {
             ...
             //making adjacent list , each index stores the ids of subordinate employees
             F_OR1(n){
                 if(manager[i]!=-1){
                 adjList[manager[i]].pb(i);
                 }
             }
...
int main(){
	...
	int n=7;
	int headID=0;
	vt<int>manager={-1,0,0,1,1,2,2};
	vt<int>informTime={0,3,3,4,5,5,4};
