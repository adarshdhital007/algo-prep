...
public:
    ...
    int shortestPathBinaryMatrix(vector<vector<int>>& grid) {
		...
		//if the start or end cell is blocked or out of bound ,return -1
		if(grid[0][0]==1 || grid[n-1][m-1]==1){
			return -1;
		}
		//create a queue for BFS and initialize it with the start cell
		...
		int pathlength=1;//length of shortest path
		...
		while(!q.empty()){
			...
			F_OR1(size){
				...
				//if we have reached the end cell, return the shortest path length
				if(row==n-1 && col==m-1){
					return pathlength;
				}

				//explore all eigth directions
				F_OR2(j,8){
...
int main(){
  Solution solution;
  vt<vt<int>>grid={{0,0,0},{1,1,0},{1,1,0}};
