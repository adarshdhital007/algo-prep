...
public:
    int findNthDigit(int n) {
        ll base = 9;
        int digits = 1; 
        //determining the length of the number where  the nth digit is located 
        while(n>digits*base){
            n-= digits * base;
            base = base * 10;
            digits++;
        ...
        //finding the actual number where the nth digit is located
        int  num = (n-1)/digits+pow(10,digits-1);

        //extracting the nth digit from the num
        int digitIndex = (n - 1)% digits;
        for(int i=digits-digitIndex-1;i>0;i--){
            num=num/10;
        }
        return num%10;
